...
class SourceDataset(Dataset):
    ...
    def _preprocess_path_contexts(self, result_path):
        ...
        for file in os.listdir(result_path):
            if file.startswith("path_contexts"):
                 path_contexts.extend(parse_line_csv(result_path / file, delimiter=" "))
...
def save_code2vec_index(dataset, path_index, fold_label):
    ...
    for (path, value) in path_index.items():
        path_index_suffix[path.split("/")[-1]] = value

    for sample in dataset:
        if sample["index"] not in path_index_suffix or len(path_index_suffix[sample["index"]]) == 0:
            logging.debug("Failure to compute code2vec paths for {}".format(sample["index"]))
            num_failures += 1
            continue

        sample["code2vec"] = path_index_suffix[sample["index"]]
        result.append(sample)
